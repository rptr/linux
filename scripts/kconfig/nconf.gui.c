...
#define COLOR_ATTR(_at, _fg, _bg, _hl)                                         \
	{                                                                      \
		.attr = &(_at), .has_color = true, .color_fg = _fg,            \
		.color_bg = _bg, .highlight = _hl                              \
	}
...
static const struct nconf_attr_param color_theme_params[] = {
	...
	COLOR_ATTR(attr_main_menu_fore, COLOR_DEFAULT, COLOR_DEFAULT,
		   A_REVERSE),
	...
	COLOR_ATTR(attr_dialog_menu_back, COLOR_YELLOW, COLOR_DEFAULT,
		   A_NORMAL),
	...
	COLOR_ATTR(attr_function_highlight, COLOR_DEFAULT, COLOR_DEFAULT,
		   A_BOLD),
	{ /* sentinel */ }
};
...
void show_scroll_win(WINDOW *main_window, const char *title, const char *text)
{
	...
	do {
		copywin(pad, win, start_y, start_x, 2, 2, text_lines, text_cols,
			0);
		print_in_middle(win, text_lines + 2, text_cols, "<OK>",
