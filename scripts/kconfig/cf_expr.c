...
static void create_fexpr_selected(struct symbol *sym)
{
	...
	struct fexpr *fexpr_sel_y =
		fexpr_create(sat_variable_nr++, FE_SELECT, sym->name);
...
static void create_fexpr_bool(struct symbol *sym)
{
	struct fexpr *fexpr_y =
		fexpr_create(sat_variable_nr++, FE_SYMBOL, sym->name);
	fexpr_y->sym = sym;
	fexpr_y->tri = yes;
...
static void create_fexpr_nonbool(struct symbol *sym)
{
	...
	for (i = 0; i < 3; i++) {
		struct fexpr *e =
			fexpr_create(sat_variable_nr++, FE_NONBOOL, sym->name);
		e->sym = sym;
		str_append(&e->name, "=");
		e->nb_val = str_new();
...
static void create_fexpr_choice(struct symbol *sym)
{
	...
	struct fexpr *fexpr_y =
		fexpr_create(sat_variable_nr++, FE_CHOICE, "Choice_");
...
static struct pexpr *gcc_version_eval(struct expr *e)
{
	...
	if (e->left.sym == sym_find("GCC_VERSION")) {
		actual_gcc_ver =
			strtoll(sym_get_string_value(e->left.sym), NULL, 10);
		sym_gcc_ver = strtoll(e->right.sym->name, NULL, 10);
	} else {
		actual_gcc_ver =
			strtoll(sym_get_string_value(e->right.sym), NULL, 10);
		sym_gcc_ver = strtoll(e->left.sym->name, NULL, 10);
	}

	switch (e->type) {
	case E_LTH:
		return actual_gcc_ver < sym_gcc_ver ? pexf(const_true) :
							    pexf(const_false);
	case E_LEQ:
		return actual_gcc_ver <= sym_gcc_ver ? pexf(const_true) :
							     pexf(const_false);
...
static struct pexpr *expr_eval_unequal_bool(struct expr *e)
{
	...
	switch (e->type) {
	case E_LTH:
		return val_left < val_right ? pexf(const_true) :
						    pexf(const_false);
	case E_LEQ:
		return val_left <= val_right ? pexf(const_true) :
						     pexf(const_false);
...
struct pexpr *expr_calculate_pexpr_both(struct expr *e)
{
	...
	switch (e->type) {
	case E_SYMBOL:
		return pexpr_or(expr_calculate_pexpr_m(e),
				expr_calculate_pexpr_y(e));
	case E_AND:
		return expr_calculate_pexpr_both_and(e->left.expr,
						     e->right.expr);
	case E_OR:
		return expr_calculate_pexpr_both_or(e->left.expr,
						    e->right.expr);
	case E_NOT:
		return pexpr_or(expr_calculate_pexpr_m(e),
				expr_calculate_pexpr_y(e));
...
{
	struct pexpr *topright = pexpr_not(pexpr_and(
		expr_calculate_pexpr_y(a), expr_calculate_pexpr_y(b)));
	struct pexpr *ll_left =
		pexpr_or(expr_calculate_pexpr_y(a), expr_calculate_pexpr_m(a));
	struct pexpr *ll_right =
		pexpr_or(expr_calculate_pexpr_y(b), expr_calculate_pexpr_m(b));
...
struct pexpr *expr_calculate_pexpr_m_or(struct expr *a, struct expr *b)
{
	...
	struct pexpr *lowerleft =
		pexpr_or(expr_calculate_pexpr_m(a), expr_calculate_pexpr_m(b));
	struct pexpr *topleft =
		pexpr_and(lowerleft, pexpr_not(expr_calculate_pexpr_y(a)));
...
{
	return pexpr_not(
		pexpr_or(expr_calculate_pexpr_y(e), expr_calculate_pexpr_m(e)));
}
...
struct pexpr *expr_calculate_pexpr_y_equals(struct expr *e)
{
	...
	if (sym_is_tristate_constant(e->left.sym) &&
		...
		return e->left.sym == e->right.sym ? pexf(const_true) :
							   pexf(const_false);
	...
	if (sym_is_bool_or_triconst(e->left.sym) &&
		...
		struct pexpr *yes = equiv_pexpr(pexf(e->left.sym->fexpr_y),
						pexf(e->right.sym->fexpr_y));
		struct pexpr *mod = equiv_pexpr(pexf(e->left.sym->fexpr_m),
						pexf(e->right.sym->fexpr_m));
	...
	if (sym_is_nonboolean(e->left.sym) && e->right.sym->type == S_UNKNOWN) {
		return pexf(sym_get_or_create_nonbool_fexpr(
			e->left.sym, e->right.sym->name));
	}
	if (e->left.sym->type == S_UNKNOWN && sym_is_nonboolean(e->right.sym)) {
		return pexf(sym_get_or_create_nonbool_fexpr(e->right.sym,
							    e->left.sym->name));
	}
...
struct pexpr *expr_calculate_pexpr_y_comp(struct expr *e)
{
	...
	case E_GEQ:
		...
		if (expr_contains_symbol(e, sym_find("CRAMFS")) &&
		    expr_contains_symbol(e, sym_find("MTD")))
...
struct pexpr *sym_get_fexpr_sel_both(struct symbol *sym)
{
	...
	return sym->type == S_TRISTATE ? pexpr_or(pexf(sym->fexpr_sel_m),
...
struct fexpr_list *fexpr_list_copy(struct fexpr_list *list)
{
	...
	fexpr_list_for_each(node, list) fexpr_list_add(ret, node->elem);
...
struct fexl_list *fexl_list_copy(struct fexl_list *list)
{
	...
	fexl_list_for_each(node, list) fexl_list_add(ret, node->elem);
...
struct sdv_list *sdv_list_copy(struct sdv_list *list)
{
	...
	sdv_list_for_each(node, list) sdv_list_add(ret, node->elem);
...
struct sfix_list *sfix_list_copy(struct sfix_list *list)
{
	...
	sfix_list_for_each(node, list) sfix_list_add(ret, node->elem);
 ...
 */
static void pexpr_eliminate_dups1(enum pexpr_type type, struct pexpr **ep1,
				  struct pexpr **ep2)
{
#define e1 (*ep1)
#define e2 (*ep2)
