...
again:
	...
	do {
		...
		set_subtitle();
		dres = show_textbox_ext("Search Results", (char *)str_get(&res),
					0, 0, keys, &vscroll, &hscroll,
	...
	if (sym_is_choice(sym)) {
		...
		if (sym_is_choice_value(sym) && val == yes) {
			...
			case S_TRISTATE:
				...
				break;
			default:
				tmp = 2 + strlen(sym_get_string_value(
						  sym)); /* () = 2 */
...
static void conf(struct menu *menu, struct menu *active_menu)
{
	...
	while (1) {
		...
		res = dialog_menu(prompt ? prompt : "Main Menu",
				  menu_instructions, active_menu, &s_scroll);
...
static void conf_choice(struct menu *menu)
{
	...
	while (1) {
		...
		res = dialog_checklist(prompt ? prompt : "Main Menu",
				       radiolist_instructions,
				       MENUBOX_HEIGTH_MIN, MENUBOX_WIDTH_MIN,
...
static void conf_string(struct menu *menu)
{
	...
	while (1) {
		...
		dialog_clear();
		res = dialog_inputbox(prompt ? prompt : "Main Menu", heading,
				      10, 75, sym_get_string_value(menu->sym));
		switch (res) {
		case 0:
			if (sym_set_string_value(menu->sym,
						 dialog_input_result))
				return;
			show_textbox(NULL, "You have made an invalid entry.", 5,
				     43);
			break;
		case 1:
			show_help(menu);
